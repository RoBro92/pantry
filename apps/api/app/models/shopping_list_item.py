...
class ShoppingListItem(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    requested_quantity: Mapped[Decimal | None] = mapped_column(Numeric(12, 3), nullable=True)
    unit: Mapped[str | None] = mapped_column(String(32), nullable=True)
    requested_unit: Mapped[str | None] = mapped_column(String(32), nullable=True)
    note: Mapped[str | None] = mapped_column(String(512), nullable=True)
    pantry_location_id: Mapped[UUID | None] = mapped_column(ForeignKey("locations.id"), nullable=True)
    ...
    pantry_location = relationship("Location")
