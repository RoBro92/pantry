...
class ProductEnrichment(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    ingredient_tags: Mapped[list[str] | None] = mapped_column(JSON(), nullable=True)
    ingredient_tokens: Mapped[list[str] | None] = mapped_column(JSON(), nullable=True)
    ...
    nutriments_payload: Mapped[dict[str, object] | None] = mapped_column(JSON(), nullable=True)
    nutrition_summary: Mapped[list[dict[str, object]] | None] = mapped_column(JSON(), nullable=True)
    nutrition_summary_text: Mapped[str | None] = mapped_column(Text(), nullable=True)
