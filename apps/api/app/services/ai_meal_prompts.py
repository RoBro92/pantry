...
def _mode_copy(request: AIMealSuggestionRequest) -> str:
    if request.pantry_only:
        return (
            "Pantro-only mode is active. Suggest meals that can be made from the pantry context only. "
    ...
    if request.allow_extra_ingredients:
        return (
            "Pantro-plus-extras mode is active. Prefer pantry stock first and keep extra ingredients to a small, practical number."
        )
