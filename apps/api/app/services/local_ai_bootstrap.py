...
def resolve_local_ai_bootstrap_config() -> LocalAIBootstrapConfig | None:
    provider_type = canonical_provider_type(_first_non_empty_env("PANTRY_LOCAL_AI_PROVIDER_TYPE"))
    if provider_type is None:
        provider_type = AI_PROVIDER_OPENAI
    if provider_type not in AI_PROVIDER_TYPES:
        return None

    base_url = _first_non_empty_env("PANTRY_LOCAL_AI_BASE_URL")
    api_key = _first_non_empty_env("PANTRY_LOCAL_AI_API_KEY")
    ...
    default_model = _first_non_empty_env("PANTRY_LOCAL_AI_DEFAULT_MODEL") or DEFAULT_LOCAL_AI_MODELS[provider_type]
