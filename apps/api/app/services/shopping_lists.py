...
def _resolve_default_pantry_location(
    ...
    *,
    ...
    if explicit_location_external_id:
        ...
        if location is None:
            raise ValueError("Pantro storage location not found.")
        return location
...
def _write_purchased_item_to_pantry(
    ...
    *,
    ...
    if item.product is None:
        raise ValueError(f"Create a Pantro product for {item.label} before finishing reconciliation.")
    ...
    if pantry_location is None:
        raise ValueError(
            f"Choose a Pantro storage location for {item.product.name} before finishing reconciliation."
        )
