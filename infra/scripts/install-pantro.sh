...
DATA_ROOT="${PANTRO_DATA_ROOT:-${PANTRY_DATA_ROOT:-$(default_data_root)}}"
...
prepare_install_layout() {
  ...
  ensure_app_data_dir_permissions "${PANTRO_DEFAULT_APP_UID}" "${PANTRO_DEFAULT_APP_GID}" \
    "${DATA_ROOT}/imports" \
    "${DATA_ROOT}/backups"
  ...
  cp "${INSTALL_DIR}/pantro.env.example" "${INSTALL_DIR}/.env"
}
...
configure_env_file() {
  ...
  env_set "${env_file}" "PANTRO_VERSION" "${selected_version}"
  env_set "${env_file}" "PANTRY_VERSION" "${selected_version}"
  env_set "${env_file}" "PANTRO_IMAGE_NAMESPACE" "${PANTRO_DEFAULT_IMAGE_NAMESPACE}"
  env_set "${env_file}" "PANTRY_IMAGE_NAMESPACE" "${PANTRO_DEFAULT_IMAGE_NAMESPACE}"
  ...
  env_set "${env_file}" "POSTGRES_DB" "pantro"
  env_set "${env_file}" "POSTGRES_USER" "pantro"
  env_set "${env_file}" "POSTGRES_PASSWORD" "${postgres_password}"
  env_set "${env_file}" "DATABASE_URL" "postgresql+psycopg://pantro:${postgres_password}@postgres:5432/pantro"
  ...
  env_set "${env_file}" "SESSION_COOKIE_NAME" "pantro_session"
  ...
  env_set "${env_file}" "PANTRO_APP_UID" "${PANTRO_DEFAULT_APP_UID}"
  env_set "${env_file}" "PANTRO_APP_GID" "${PANTRO_DEFAULT_APP_GID}"
  ...
  env_set "${env_file}" "PANTRY_APP_UID" "${PANTRO_DEFAULT_APP_UID}"
  env_set "${env_file}" "PANTRY_APP_GID" "${PANTRO_DEFAULT_APP_GID}"
...
${PANTRO_COLOR_GREEN}Pantro install complete.${PANTRO_COLOR_RESET}
...
Useful commands:
  ...
  docker compose --env-file .env -f pantro.yml logs -f web
  docker compose --env-file .env -f pantro.yml logs -f api
  docker compose --env-file .env -f pantro.yml logs -f worker
  ./healthcheck-pantro.sh --install-dir ${INSTALL_DIR}
  ./update-pantro.sh --install-dir ${INSTALL_DIR}

First-run setup:
  Open ${browser_url}/ in a browser
  or run:
  docker compose --env-file .env -f pantro.yml run --rm api \\
...
Legacy upgrade aliases are installed too:
  pantry.yml, pantry.env.example, update-pantry.sh, healthcheck-pantry.sh
EOF
}
...
main() {
  ...
  install_helper_scripts "${selected_version}"
  run_install
