...
PANTRO_SELFHOST_LIB_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
...
if [[ -t 1 ]]; then
  ...
  PANTRO_COLOR_GREEN=$'\033[1;32m'
  PANTRO_COLOR_YELLOW=$'\033[1;33m'
  PANTRO_COLOR_RED=$'\033[1;31m'
  PANTRO_COLOR_RESET=$'\033[0m'
else
  PANTRO_COLOR_BLUE=""
  PANTRO_COLOR_GREEN=""
  PANTRO_COLOR_YELLOW=""
...
log_step() {
  printf '%s==>%s %s\n' "${PANTRO_COLOR_BLUE}" "${PANTRO_COLOR_RESET}" "$*"
}

log_info() {
  printf '%s[info]%s %s\n' "${PANTRO_COLOR_GREEN}" "${PANTRO_COLOR_RESET}" "$*"
}

log_warn() {
  printf '%s[warn]%s %s\n' "${PANTRO_COLOR_YELLOW}" "${PANTRO_COLOR_RESET}" "$*" >&2
}

log_error() {
  printf '%s[error]%s %s\n' "${PANTRO_COLOR_RED}" "${PANTRO_COLOR_RESET}" "$*" >&2
}
...
read_local_version() {
  if [[ -f "${PANTRO_REPO_ROOT}/VERSION" ]]; then
    tr -d '[:space:]' < "${PANTRO_REPO_ROOT}/VERSION"
  fi
}

default_install_dir() {
  if [[ -d /opt/pantro ]]; then
    printf '/opt/pantro'
    return 0
  fi
...
default_data_root() {
  if [[ -d /srv/pantro ]]; then
    printf '/srv/pantro'
    return 0
  fi

  if [[ -d /srv/pantry ]]; then
    printf '/srv/pantry'
    return 0
  fi
...
resolve_install_compose_file() {
  ...
  if [[ -f "${install_dir}/pantro.yml" ]]; then
    printf '%s/pantro.yml' "${install_dir}"
    return 0
  fi

  if [[ -f "${install_dir}/pantry.yml" ]]; then
    printf '%s/pantry.yml' "${install_dir}"
    return 0
  fi
...
resolve_install_env_template_file() {
  local install_dir="$1"

  if [[ -f "${install_dir}/pantro.env.example" ]]; then
    printf '%s/pantro.env.example' "${install_dir}"
    return 0
  fi

  if [[ -f "${install_dir}/pantry.env.example" ]]; then
    printf '%s/pantry.env.example' "${install_dir}"
    return 0
  fi
...
docker_compose_in_dir() {
  ...
  docker compose --env-file "${install_dir}/.env" -f "$(resolve_install_compose_file "${install_dir}")" "$@"
}
