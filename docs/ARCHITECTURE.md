...
Pantro is a small repo for a self hosted product with three runtime services and a shared deployment layer.
...
- PostgreSQL stores Pantro domain and configuration data
...
- Pantro does not ship hosted control-plane, billing, or other SaaS-only logic in this repository
- Household access is enforced server side
- Product identity stays Pantro owned even when external enrichment from Open Food Facts is linked
...
- Setup supports both a fresh install path and restore from a Pantro backup bundle
...
- `infra/compose/pantro.yml`: released self-hosted stack
- `infra/env/pantro.env.example`: released environment template
