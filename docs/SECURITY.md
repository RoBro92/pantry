...
Pantro treats uploads, restore bundles, and external data as hostile input until they have been validated.
...
- Pantro records remain Pantro owned even when external enrichment is attached
...
- Restore accepts Pantro backup bundle JSON files only
...
- External enrichment is advisory metadata, not Pantro’s canonical product record
- Pantro stores selected product facing enrichment fields instead of treating upstream payloads as trusted source data
...
This repository documents Pantro’s application level safety model for the self-hosted, operator-managed product. Host hardening, TLS, backup retention, secret management, and patching remain operator responsibilities.
